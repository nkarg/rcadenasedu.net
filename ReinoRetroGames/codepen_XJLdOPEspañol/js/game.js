...
function initGame() {
  // Config vars
  //if (document.getElementById("radio-easy").checked == true) {
  //    var animSpeed = 500;
  //} else if (document.getElementById("radio-hard").checked == true) {
  //    var animSpeed = 100;
  //}
  var animSpeed = 100;
  ...
  function end() {
    ...
    $game.on('click', '.cup', function()
    {
      ...
      if(posBall === posStart)
      {
        ...
        $gameResult.removeClass("failure");
				$gameResult.addClass("success");
        $gameResult.text('Buenaaa! KLEIN SE LA COME');
      }
      else {
        // If Ball Not Found
        var names = [
            'No richard no es asi',
            'FRIENDZONE!?',
            'ETEEEEE',
            'DEJA DE ROBAR PAPUH'
        ];

        resultMessage = names[Math.floor(Math.random() * names.length)];
        
        $gameResult.removeClass("success");
        $gameResult.addClass("failure");
        $gameResult.text(resultMessage);
      }
